...
public class InitBlockBeforeFunctionRule : ZenhelixRule(
    ruleId = RuleId("zenhelix:init-block-before-function"),
) {
    ...
    override fun beforeVisitChildNodes(
        ...
        for (child in node.getChildren(RELEVANT_TYPES)) {
            when (child.elementType) {
                KtNodeTypes.FUN -> functionSeen = true

                KtNodeTypes.CLASS_INITIALIZER -> if (functionSeen) {
                    emit(
                        child.startOffset,
                        "Initializer block should be declared before function declarations",
                        false,
                    )
                }
            }
